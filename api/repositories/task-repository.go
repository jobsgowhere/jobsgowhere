...
func (repo *TaskRepository) CreateTask(task models.Task) (models.Task, error) {
	statement := `
    insert into tasks (userid, title, due, completed, effort, notes)
    values ($1, $2, $3, $4, $5, $6)
    returning id
  `
	var id int64
	err := repo.db.QueryRow(statement, task.UserID, task.Title, task.Due, task.Completed, task.Effort, task.Notes).Scan(&id)
	if err != nil {
		return task, err
	}
	createdTask := task
...
func (repo *TaskRepository) UpdateTask(userid string, id int64, task models.Task) error {
	query := `
    update tasks
    set title=$3, due=$4, completed=$5, effort=$6, notes=$7
    where userid=$1 and id=$2
  `

	res, err := repo.db.Exec(query, userid, id, task.Title, task.Due, task.Completed, task.Effort, task.Notes)
	if err != nil {
		return err
	}
